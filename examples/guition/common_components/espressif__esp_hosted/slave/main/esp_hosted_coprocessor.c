...
/* Weak default: ETH_IF packets are silently dropped unless overridden */
__attribute__((weak)) void esp_hosted_coprocessor_eth_rx(const uint8_t *payload, uint16_t len)
{
...
static void process_rx_pkt(interface_buffer_handle_t *buf_handle)
{
	...
	if (buf_handle->if_type == ESP_STA_IF && station_connected) {
		...
		esp_hosted_coprocessor_eth_rx(payload, payload_len);
	}
...
{
	static uint16_t eth_seq_num = 0;

	if (!data || !len || !datapath) {
	...
	buf_handle.if_type = ESP_ETH_IF;
	buf_handle.if_num = 0;
	buf_handle.seq_num = eth_seq_num;
	buf_handle.flag = 0;
	buf_handle.payload = buf;
	buf_handle.payload_len = len;
