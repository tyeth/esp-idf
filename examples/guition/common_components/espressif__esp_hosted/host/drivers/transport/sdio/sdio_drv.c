...
static void sdio_process_rx_task(void const* pvParameters)
{
	...
	while (1) {
		...
		if (buf_handle->if_type == ESP_SERIAL_IF) {
			...
			/* Forward ETH_IF packets to the registered application callback */
			extern void esp_hosted_eth_if_rx_internal(const uint8_t *payload, uint16_t len);
			esp_hosted_eth_if_rx_internal(buf_handle->payload, buf_handle->payload_len);
		} else {
